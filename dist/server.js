...
// Minting guardrails
const mint_rate_limit_middleware_1 = require("./middleware/mint-rate-limit.middleware");
const mint_gating_middleware_1 = require("./middleware/mint-gating.middleware");
const mint_error_handler_1 = require("./middleware/mint-error-handler");
const treasury_monitor_service_1 = require("./services/treasury-monitor.service");
...
const woo_hmac_middleware_1 = require("./middleware/woo-hmac.middleware");
// Capture raw body for webhook routes BEFORE JSON parsing
app.use('/wc/webhooks', woo_hmac_middleware_1.rawBodyMiddleware);
app.use('/webhooks', woo_hmac_middleware_1.rawBodyMiddleware);
...
// ============================================
const routeStatus = {};
function safeLoadRoute(name, path, loader) {
    try {
        const route = loader();
...
const tolaArtRoutes = safeLoadRoute('tola-art', '/api/tola', () => require('./routes/tola-art.routes').default);
...
// ============================================
let usdcService = null;
let tolaService = null;
let nftService = null;
let dailyAssetService = null;
let tolaMetricsService = null;
let paymentService = null;
let webhookProcessor = null;
let collectionService = null;
let marketplaceService = null;
let creatorService = null;
let royaltyService = null;
// Initialize services
try {
...
let treasuryMonitor = null;
try {
    const treasuryPubkey = nftService?.getTreasuryAddress?.() || null;
    if (treasuryPubkey) {
        treasuryMonitor = new treasury_monitor_service_1.TreasuryMonitorService(treasuryPubkey);
...
app.get('/health', async (req, res) => {
    ...
    if (detailed) {
        ...
        if (usdcService?.getHealth) {
            try {
                response.detailed.usdc = await usdcService.getHealth();
            }
            catch (e) { }
        }
        if (tolaService?.getHealth) {
            try {
                response.detailed.tola = await tolaService.getHealth();
            }
            catch (e) { }
        }
        if (nftService?.getHealth) {
            try {
                response.detailed.nft = await nftService.getHealth();
            }
            catch (e) { }
        }
        if (webhookProcessor?.getStats) {
            try {
                response.detailed.webhooks = webhookProcessor.getStats();
            }
            catch (e) { }
        }
    }
...
app.get('/tola/stats', async (req, res) => {
    const stats = { success: true, version: '4.0.0', timestamp: new Date().toISOString() };
    if (tolaService?.getStats)
        stats.distribution = tolaService.getStats();
    if (nftService?.getStats)
        stats.nfts = nftService.getStats();
    if (tolaMetricsService?.getStats)
        stats.metrics = tolaMetricsService.getStats();
    res.json(stats);
});
// ============================================
// USDC ENDPOINTS
...
app.post('/api/usdc/transfer', async (req, res) => {
    if (!usdcService)
        return res.status(503).json({ success: false, error: 'USDC service unavailable' });
...
app.post('/api/tola/transfer', async (req, res) => {
    if (!tolaService)
        return res.status(503).json({ success: false, error: 'TOLA service unavailable' });
...
// ============================================
app.post('/api/tola/mint-nft', mint_error_handler_1.attachRequestId, mint_rate_limit_middleware_1.mintRateLimiter, mint_gating_middleware_1.mintGating, async (req, res) => {
    ...
    if (!name && !metadata?.name)
        return res.status(400).json({ ok: false, success: false, code: 'VALIDATION_ERROR', message: 'name is required', request_id: requestId });
    if (!uri && !metadata?.image)
        return res.status(400).json({ ok: false, success: false, code: 'VALIDATION_ERROR', message: 'uri is required', request_id: requestId });
    // Pre-mint treasury balance check
    if (treasuryMonitor) {
        const blocked = await treasuryMonitor.preMintCheck(requestId);
        if (blocked)
            return res.status(503).json(blocked);
    }
    try {
        ...
        if (result.success) {
            ...
            const { httpStatus, body } = (0, mint_error_handler_1.buildSafeErrorResponse)(new Error(result.error || 'Mint failed'), requestId);
            res.status(httpStatus).json(body);
        }
    }
    catch (error) {
        console.error('[MINT ERROR]', error.message);
        const { httpStatus, body } = (0, mint_error_handler_1.buildSafeErrorResponse)(error, requestId);
        res.status(httpStatus).json(body);
    }
});
app.post('/api/tola/transfer-nft', async (req, res) => {
    if (!nftService)
        return res.status(503).json({ success: false, error: 'NFT service unavailable' });
    const { mint_address, recipient_wallet } = req.body;
    if (!mint_address || !recipient_wallet)
        return res.status(400).json({ success: false, error: 'mint_address and recipient_wallet are required' });
    try {
...
app.post('/api/payments/create-intent', async (req, res) => {
    ...
    if (!order_id || !amount_usd)
        return res.status(400).json({ success: false, error: 'order_id and amount_usd are required' });
    try {
...
app.post('/api/payments/verify', async (req, res) => {
    ...
    if (!signature || !order_id)
        return res.status(400).json({ success: false, error: 'signature and order_id are required' });
    try {
...
app.get('/tola/payments/status/:orderId', async (req, res) => {
    if (!paymentService)
        return res.json({ success: true, data: { orderId: req.params.orderId, status: 'pending' } });
    try {
...
const createWebhookHandler = (eventType) => async (req, res) => {
...
app.post('/api/royalty/webhook/sale', async (req, res) => {
    ...
    if (royaltyService && req.body.type === 'NFT_SALE') {
        try {
            ...
            const payment = await royaltyService.processSecondarySale(nft?.mint || '', signature || '', parseFloat(amount || '0'), buyer, seller);
...
app.get('/api/webhooks/stats', (req, res) => {
    if (!webhookProcessor)
        return res.status(503).json({ success: false, error: 'Webhook processor unavailable' });
